...
| WG827 router admin | Working | SSH/LuCI: `root` / `***REMOVED***`, WiFi AP broadcasting |
...
Indro Robotics shipped a [ZBT WG827 router](hardware.md) velcroed to the robot's back. We got the admin password (`root` / `***REMOVED***`) but haven't seriously configured it yet. This is likely the intended path to proper connectivity.

### What the WG827 can do

- **WiFi AP on 192.168.123.0/24** - already broadcasting as "UnitreeG1-Router" (password "***REMOVED***"). Since it's plugged into the G1's internal switch, WiFi clients would be on the same subnet as the Jetson (.164) and locomotion computer (.161). This bypasses the separate-subnet problem entirely.
