...
|-----------|--------|-------|
| Wired SSH to Jetson | Working | USB ethernet adapter, static IP 192.168.123.100, `ssh unitree@192.168.123.164`, password `***REMOVED***` (note: Jetson IP varies per boot, was .13 previously, now .164) |
...
| WG827 router admin | Working | SSH: `root` / `***REMOVED***`, WiFi SSID: "UnitreeRouter" / "***REMOVED***" (2.4GHz ch11, WPA2) |
...
- WG827 router WiFi "UnitreeRouter" / "***REMOVED***" (2.4GHz, WPA2, no special characters)
...
```bash
# SSH into router
sshpass -p '***REMOVED***' ssh -o StrictHostKeyChecking=no root@192.168.123.1
...
1. Set up hostapd on Jetson wlan0 (2.4GHz, channel 6, WPA2, password "***REMOVED***")
